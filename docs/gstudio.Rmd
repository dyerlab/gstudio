...
This should get you everything you need. The **gstudio**  package does contain a lot of build-in documentation including a lot of examples.  All the functions and examples associated with them can be found in the build-in documents available from:
...
```{r}
Gst( arapat$Population, arapat$LTRS, nperm=99 )
```
...
```{r}
Gst( arapat$Population, arapat[,c(7:14)], nperm=99)
```
...
Gst_prime(arapat$Population, arapat$MP20, nperm=99 )
```

#### Joost's $D_{est}$ Parameter

```{r}
Dest(arapat$Population, arapat$MP20, nperm=99 )
```
...
For simplicity, I'll use the cape region clade as an example here and borrow the AMOVA routine from the **ecodist** package.
