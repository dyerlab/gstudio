...
Here are some examples of data files with different kinds of genetic data, each of which exercises the `read.population()` function in a different way.  Hopefully this covers the main types of data being imported, if not, drop me an email (rjdyer@vcu.edu).  Missing genotypes should be missing data or encoded as `NA`.  If you do not have a genotype then leave it blank.  There is no reason to use negative numbers or other conventions.  It is missing
