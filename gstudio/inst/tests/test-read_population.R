...
test_that("reading two-column csv file", {
  ...
  expect_that( data <- read.population(path), gives_warning() )
  
  # bad locus.columns
  expect_that( (data <- read.population(path, locus.columns="BOB")), throws_error() )
  
  # real load
  data <- read.population( path, locus.columns=seq(5,62,by=2), is.two.column=TRUE)
...
test_that("reading aflp data file",{
  ...
  data <- read.population(path,locus.columns=seq(2,141), sep=",", is.aflp=TRUE)
