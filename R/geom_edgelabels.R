...
geom_edgelabels <- function(
  ...
  vals <- edge_attr(graph, as.character(mapping$label)[2])
  vals <- format(vals, digits = 4)
  df.lbls <- data.frame(
    X = (df$X1 + df$X2) / 2 + offset[1],
    Y = (df$Y1 + df$Y2) / 2 + offset[2],
    label = format(vals, digits = 4)
  )

  ret <- ggplot2::geom_text(
