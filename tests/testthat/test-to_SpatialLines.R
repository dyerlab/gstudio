test_that("to_SpatialLines is deprecated and still works", {
  ...
  expect_warning(to_SpatialLines(graph), "deprecated")

  sl <- suppressWarnings(to_SpatialLines(graph))
  expect_true(is(sl, "SpatialLines"))
  bbexp <- matrix(c(-122.89, -77.16, 37.74, 47.15), ncol = 2, byrow = TRUE)
  expect_equivalent(bbox(sl), bbexp)
})
